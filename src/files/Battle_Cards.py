...
if st.session_state.logged_in and st.session_state.username == 'admin': 
    ...
    cols2 = st.columns(2)
    with cols2[0]:
        battlecard_name = st.text_input("Comeptitor Name", placeholder="e.g., Databricks, Redshift...", key='battlecard_name')

    with cols2[1]:
        battlecard_industry = st.text_input("Comeptitor Industry", placeholder="e.g., Telecommunications...", key='battlecard_industry')
...
\nSnowflake Response:\n{battlecard_snowflake_response}
    ...
    st.info("Only administrators can manage battle cards.")
